...
df = yf.download('AMZN', start = '2018-01-01', end = '2026-01-03', progress = False)
...
def fmt(label, log_ret):
    factor = np.exp(log_ret)
    pct = factor - 1
    print(
        f"{label:<32}"
        f"log: {log_ret:>7.3f} | "
        f"factor: {factor:>6.2f}x | "
        f"return: {pct:>7.1%}"
    )

print(f"\nBest MA window: {best_w}\n")

fmt("Train strategy", train_ret)
fmt("Test strategy", test_ret)
fmt("Full strategy (best MA)", max(returns_list) + test_ret)

fmt("Buy & Hold (test only)", df['LogReturn'].iloc[-500:].sum())
fmt("Buy & Hold (full period)", df['LogReturn'].sum())
