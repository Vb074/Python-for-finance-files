...
def down_out_call_CN_scipy(S0=10, E=10, r=0.01, q=0.005, sigma=0.3, T=1.0, B=9.0, Smax=40.0, M=300, N=600):
    ...
    return price, S, V, aj, bj, cj, dt

price, S, V, aj, bj, cj, dt = down_out_call_CN_scipy()
print("Down-and-out call price:", price)

def von_Neuman(a,b,c,dt,ntheta=1000):
    theta = np.linspace(0, np.pi, ntheta)
    sym = a*np.exp(-1j * theta) + b + c*np.exp(1j * theta)
    X_i = (1 + 0.5*dt*sym) / (1 - 0.5*dt*sym)
    return np.max(np.abs(X_i))

M = 300
j = M // 2
eps = von_Neuman(aj[j-1], bj[j-1], cj[j-1], dt)

print('Maximum Amplification Factor: ', eps)
plt.plot(S, V)
plt.xlabel("S")
plt.ylabel("V(S,0)")
plt.title("Down-and-out call via Crank–Nicolson")
plt.show()
print('Since the maximum amplification factor is <= 1 for all Fourier modes, the Crank–Nicolson scheme is unconditionally stable in the von Neumann sense')
